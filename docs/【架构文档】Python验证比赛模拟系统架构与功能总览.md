...
### 尚未探索的方向
4. **现有的只带短传球的框架的修正与改进**
5. **犯规与定位球**：无犯规机制，无任意球/点球。
6. **高空球**：无高空过顶长传设定。
7. **阵容多样化**：仅支持 1-2-1（GK-ANCHOR-LEFT/RIGHT-PIVOT），不支持变阵。
8. **球员间纵向间距下限**：仅有"太近推开"，没有"太远拉近"的吸引力。
9. **防守横向压缩**：防守时球队不主动横向收窄。
10. **Zonal marking**：所有盯人是 man-oriented 找最危险对手，无区域防守。
11. **参数自动调优**：46 个参数全部手工设定。
12. **批量模拟框架**：无参数扫描/数据收集工具。
